...
int main() {
    ...
    for (k = 0; k < N; k++) {
        ...
        else if (abs(L.x1[k] - L.x2[k]) == abs(L.y1[k] - L.y2[k])) {
            int incx = (L.x1[k] < L.x2[k]) ? 1 : -1;
            int incy = (L.y1[k] < L.y2[k]) ? 1 : -1;
            for (j = L.x1[k], i = L.y1[k]; j != L.x2[k]; j += incx, i += incy)
                matrix[i][j]++;
            /* ultimo incremento no ponto (x2, y2) */
            matrix[L.y2[k]][L.x2[k]]++;
        }
    }
