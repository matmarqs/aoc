...
ll bin2dec(char *bin, int binlen) {
    ll dec = 0LL, fac = 1LL;
    for (ll i = (ll) binlen - 1LL; i >= 0LL; i--, fac *= 2LL)
        dec += ((ll) (bin[i] - '0')) * fac;
    return dec;
}
...
int val_loop(int i, int pos, ll num, int id) {
    if (id)
        return (i < num);
    else
        return (pos - 22 < num);
...
ll val_packet(char *bin, int *total_len) {
    ...
    if (type == 4) {
        ...
        for (int j = 0; j < num_groups; j++) {
            for (int until4 = 0; until4 < 4; until4++)
                lit[4*j + until4] = bin[7 + 5 * j + until4];
        }
        lit[litlen] = '\0'; /* end of string */
        val = bin2dec(lit, litlen);
        *total_len += 6 + 5 * num_groups;
    }
    else {
        int id = bin[6] - '0';
        char length_str[16];    /* at most 15 bits for total length + 1 for '\0' */
        int length_str_len, pos;
        if (id) /* next 11 bits represent the number of sub-packets */
            length_str_len = 11, pos = 18;
        else    /* next 15 bits are the total length in bits of the sub-packets */
            length_str_len = 15, pos = 22;
        strcpy_pos(bin, length_str, 7, length_str_len);
        ll num = bin2dec(length_str, length_str_len), aux, aux2;
        int i;
        switch (type) {
            case 0:
                val = 0LL;
                for (i = 0; val_loop(i, pos, num, id); i++)
                    val += val_packet(bin + pos, &pos);
                break;
            case 1:
                val = 1LL;
                for (i = 0; val_loop(i, pos, num, id); i++)
            ...
            case 2:
                val = val_packet(bin + pos, &pos);
                for (i = 1; val_loop(i, pos, num, id); i++) {
                    aux = val_packet(bin + pos, &pos);
                    if (val > aux) val = aux;
                }
                break;
            case 3:
            ...
            case 5:
                aux  = val_packet(bin + pos, &pos);
                aux2 = val_packet(bin + pos, &pos);
                val = (aux > aux2) ? 1LL : 0LL;
                break;
            case 6:
                aux  = val_packet(bin + pos, &pos);
                aux2 = val_packet(bin + pos, &pos);
...
int main(int argc, char *argv[]) {
    ...
    /* solving the problem */
    int total_len = 0;
    ll val = val_packet(line_bin, &total_len);
