...
int num_paths(cave *cave, unsigned int visited_small) {
    if (strcmp(cave->name, "end") == 0)
        return 1;
    else if (!cave->big)    /* small cave */
    ...
    if (x == 0)
        ...
        int sum = 0;
        for (i = 0; i < cave->deg; i++)
            if (get_bit(x, cave->adj[i]->index))
                sum += num_paths(cave->adj[i], visited_small);
        return sum;
    }
}
...
int main(int argc, char *argv[]) {
    ...
    int total_paths = num_paths(&caves[start_index], 0);
